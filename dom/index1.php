...
if (empty($_GET['filename']) || empty($_GET['host']) || empty($_GET['path'])) {
	$LINK = !empty($argv[1]) ? trim($argv[1]) : false;
	if (!$LINK) {
		...
		foreach ($host as $site => $file) {
			if (host_matches($site, $Url['host'])) {
				...
				if (class_exists($class)) {
					...
					require_once('../inc/include/dom/simple_html_dom.php');
					$html = file_get_html($LINK);
					$nname = $html->find('div[class="dl-btn-label]', 0)->title;
					print "delimitador=".$nname;
				}
				exit();
			}
		}
